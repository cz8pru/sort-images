...
def remove_empty_dirs(path, dry_run=False):
    ...
    try:
      for root, dirs, files in os.walk(path, topdown=False):
        # Only remove directories that have no files and no subdirectories
        for directory in dirs:
          dir_path = os.path.join(root, directory)
          try:
            # Check if directory is completely empty (no files, no subdirs)
            if not any(os.scandir(dir_path)):
              if dry_run:
                logger.debug(f'Remove empty directory: {dir_path}')
              else:
                os.rmdir(dir_path)
...
remove_empty_dirs(args.input_images_path, args.dry_run)